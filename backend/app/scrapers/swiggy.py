"""
Swiggy Instamart scraper — uses Playwright browser with geolocation.
...
() => {
    ...
    for (const priceEl of priceEls) {
        ...
        let name = '';
        const skip = ['ADD', 'Add', 'ADDED', 'OUT OF STOCK', 'OFF'];
        ...
        if (name && price) {
            results.push({
                ...
                price,
                image_url: null,
                product_url: href.startsWith('http') ? href : 'https://www.swiggy.com' + href,
            });
        }
    }
...
class SwiggyScraper(BaseScraper):
    ...
    async def search(self, query: str, lat: str = "28.6315", lon: str = "77.2167", **kwargs) -> List[dict]:
        ...
        try:
            ...
            logger.info(f"[swiggy] {url}")
            await page.goto(url, wait_until="domcontentloaded", timeout=25000)
            await page.wait_for_timeout(4000)

            content = await page.content()
            if '₹' not in content:
                logger.warning("[swiggy] No price content — possibly location gate or blocked")
                return []

            products = await page.evaluate(_EXTRACT_JS)
            logger.info(f"[swiggy] {len(products)} products")
            ...
            results = [
                self.build_result({
                    ...
                    "unit": None,
                    "image_url": None,
                    "product_url": p.get("product_url", "https://www.swiggy.com/instamart"),
                    "in_stock": True,
                    "discount_pct": None,
                })
                for p in products
            ...
            logger.warning(f"[swiggy] scrape failed: {e}")
        finally:
            await context.close()
        return results
